...
mask = ctx.clip
image = (img,x,y,w=img.width,h=img.height)=>img instanceof Promise ? await img.then(loadedImage => ctx.drawImage(loadedImage, x || 0, y || 0, w || loadedImage.width, h || loadedImage.height)) :  ctx[img instanceof ImageData ? 'putImageData' : 'drawImage'](img, x || 0, y || 0, w || img.width, h || img.height)
loadImage = src => new Promise((resolve, reject) => {
    const img = Object.assign(new Image, {src: src, onerror: () => reject('invalid or unaccessible image source')})
