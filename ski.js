...
credits:
	...
	PT - khanacademy.org/profile/kaid_1007254215278267494550608 - helping to incorporate requestAnimationFrame over setInterval as well for the structure of the get function	Processing.js - the use of the radius parameter in the rect function an' its related code
...
arc = (x,y,w,h,start,stop,close=false)=>{
	...
	ctx.arc(x, y, Math.min(w, h) / 2, degrees(start), degrees(stop))
...
ellipse = (x,y,w,h)=>{
	...
	ctx.arc(x, y, Math.min(w, h) / 2, Math.PI * 2, 0)
...
image = (img,x,y,w=img.width,h=img.height)=>ctx[img instanceof ImageData ? 'putImageData' : 'drawImage'](img, data.image ? x - w / 2 : x, data.image ? y - h / 2 : y, w, h)

loadImage = (src, width, height) => (/khanacademy/).test(src) ? Object.assign(new Image(width, height), {src: src}) : Object.assign(new Image(width, height), {src: src, crossOrigin: 'anonymous'})
